...
public class VchPrintService {
    ...
    public void printVch(String title, String vchset, String txndat,
        ...
        for (Vchset vch : vchList) {
            String row = StringPad.pad4ChineseToByteLength(true, vch.getSETSEQ(), 3, " ") +         //序号
                    StringPad.pad4ChineseToByteLength(true, "    " + vch.getACTNUM(), 20, " ")+    //账号
                    StringPad.pad4ChineseToByteLength(true, "    " + vch.getRVSLBL(), 6, " ")+      //标志
                    StringPad.pad4ChineseToByteLength(true, "    " + vch.getTXNAMT(), 22, " ")+     //金额
                    StringPad.pad4ChineseToByteLength(true, "    " + vch.getFURINF(), 8, " ")+     //摘要
                    StringPad.pad4ChineseToByteLength(true, "    " + vch.getVALDAT(), 12, " ")+     //日期
                    StringPad.pad4ChineseToByteLength(true, "    " + vch.getANACDE(), 15, " ") + vch.getVCHATT();
    ...
    private PdfPTable initVchPdfPTable(String title, String vchset,  String txndat,
        ...
        table.setSpacingBefore(170f);// 设置表格上面空白宽度
        table.setTotalWidth(875);// 设置表格的宽度
...
//        String[] labels = new String[]{"aaaaa", "bbbbb", "ccccc", "ddddd", "eeeee"};
        ...
        String row1 = StringPad.pad4ChineseToByteLength(true, title, 55, " ");

        String row2 = StringPad.pad4ChineseToByteLength(true, " = =  = = = = = = = = = = = = = = " +
                "= = = = = = = = = = = = = = = = =", 78, " ");

        String row3 = StringPad.pad4ChineseToByteLength(true, "日期：", 10, " ") + txndat +
                StringPad.pad4ChineseToByteLength(true, "套  号：", 61, " ") + vchset;

        String row4 = StringPad.pad4ChineseToByteLength(true, "时间：", 10, " ") + txntim +
                StringPad.pad4ChineseToByteLength(true, "柜员号：", 63, " ") + om.getOperatorId();

        String row5 = StringPad.pad4ChineseToByteLength(true, "- - - - - - - - - - - - - - - - - - - - - " +
                "- - - - - - - - - - - - - - - " +
                "- - - - - - - - - - - - - -"  , 98, " ");

        String row6 = StringPad.pad4ChineseToByteLength(true, "序号", 4, " ") +
                StringPad.pad4ChineseToByteLength(true, "账号", 12, " ") +
                StringPad.pad4ChineseToByteLength(true, "标志", 14, " ") +
                StringPad.pad4ChineseToByteLength(true, "金额", 16, " ") +
                StringPad.pad4ChineseToByteLength(true, "摘要", 13, " ") +
                StringPad.pad4ChineseToByteLength(true, "日期", 10, " ") +
                StringPad.pad4ChineseToByteLength(true, "统计码", 12, " ") +
                StringPad.pad4ChineseToByteLength(true, "附件", 8, " ");
